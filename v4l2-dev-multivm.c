...
static int init_virtio_socket(char *socket_path, void *buf, unsigned long len) {
		int retval;
	  char* string = "P6\n640 480 255\n";
          struct socket *sock = NULL;
	  ...
	  mm_segment_t oldfs;
	  printk (KERN_INFO "Inside init virtio socket\n");
	  // create
		  printk(KERN_INFO "Creating a socket\n");
	  ...
	  if(retval == -1){
	  	printk(KERN_INFO "Initial Socket Send Message failed\n");
	  	return -1;
	  }
	  printk(KERN_INFO "Sending the super message! Brace yourselves!\n");
          retval = sock_sendmsg(sock, &msg2, len);
          if(retval == -1){
                printk(KERN_INFO "Super Socket Send Message failed\n");
                return -1;
          }

	  printk(KERN_INFO "After socket send. Things are looking fine, or are they?\n");
	  set_fs(oldfs);
	  sock_release(sock);
	  sock = NULL;
	  //sock = NULL;
	  //printk(KERN_INFO " NULLED ");
	  return 0;
}
		...
		size_t sz, loff_t *off)
{
	...
	int ret = -ENODEV;
	printk(KERN_INFO "Inside v4l2 read function");
	dump_stack();
	if (!vdev->fops->read)
		return -EINVAL;
	if (video_is_registered(vdev)) {
		printk(KERN_INFO "Writing to device buffer with size %d", (int) sz);
		printk(KERN_INFO "Buffer character 1: %c", buf[0]);
		ret = vdev->fops->read(filp, buf, sz, off);
		init_virtio_socket("/tmp/foo",buf, sz);
                printk(KERN_INFO "Done with the first VM. Moving on to the 2nd one\n");
                init_virtio_socket("/tmp/foo2",buf, sz);
                printk(KERN_INFO "Done with both VMs. May the Gods be with us\n");
	}
	if (vdev->debug)
		...
		size_t sz, loff_t *off)
{
	...
	if (!vdev->fops->write)
		return -EINVAL;
	if (video_is_registered(vdev)) {
		printk(KERN_INFO "Called v4l2_write");
		...
		unsigned long flags)
{
	struct video_device *vdev = video_devdata(filp);
	int ret;
	printk(KERN_INFO "Unmapped area called");
	pringk(KERN_INFO "V4L2: ADDR: %lu, LEN: %lu, PGOFF: %lu, FLAGS: %lu \n", addr, len, pgoff, flags);
...
static int v4l2_mmap(struct file *filp, struct vm_area_struct *vm)
{
	...
	char *p;
	unsigned long size;
	printk(KERN_INFO "Inside the mmap function \n");
	dump_stack();
	if (!vdev->fops->mmap)
		return -ENODEV;
	if (video_is_registered(vdev)) {
		printk(KERN_INFO "Calling mmap of v4l2_mmap");
		ret = vdev->fops->mmap(filp, vm);
	}
	if (vdev->debug)
		printk(KERN_DEBUG "%s: mmap (%d)\n",
			video_device_node_name(vdev), ret);
	printk(KERN_INFO " VM START: %lu and END: %lu\n", vm->vm_start, vm->vm_end);
	size = vm->vm_end - vm->vm_start;
	ptr = kmalloc(size, GFP_ATOMIC);
	p = vm->vm_start;
	copy_from_user(ptr, (void *)p, size);
	printk(KERN_INFO "Not going to call init_virtio_socket in mmap\n");
	printk(KERN_INFO "First byte of the buffer: %c",*((char *)ptr));
	//init_virtio_socket(ptr, size);
	printk(KERN_INFO "Returned back from the Virtio Socket call back to MMAP");
	kfree(ptr);
	ptr = NULL;
	return ret;
}
...
static int v4l2_open(struct inode *inode, struct file *filp)
{
	...
	int ret = 0;
	printk(KERN_INFO "V4L2 open called!! YAAY!\n");
	/* Check if the video device is available */
	mutex_lock(&videodev_lock);
	vdev = video_devdata(filp);
	printk(KERN_INFO "opening v4l2 device\n");
	...
	if (ret)
		video_put(vdev);
	//printk(KERN_INFO " CALLING virtio socket\n");
	//init_virtio_socket();
	return ret;
}
